...
namespace PubPay.Common.Alipay
{
    ...
    public class AlipaySubmit
    {
        ...
        //商户的私钥文件
        private static string _private_key_file = string.Empty;
        //编码格式
        ...
        static AlipaySubmit()
        {
            _key = AlipayConfig.MD5Key.Trim();
            _private_key = AlipayConfig.RSAKey;
            _private_key_file = AlipayConfig.RSAKeyFile;
            _input_charset = AlipayConfig.InputCharset.Trim().ToLower();
            _sign_type = AlipayConfig.SignType.Trim().ToUpper();
        }
        ...
        private static string BuildRequestMysign(Dictionary<string, string> sPara)
        {
            ...
            switch (_sign_type)
            {
                ...
                case "RSA":
                    ...
                    {
                        mysign = RSAFromPkcs8.RSASign(prestr, _private_key, _input_charset);
                        break;
                    }
                    if (!string.IsNullOrEmpty(_private_key_file))
                    {
                        mysign = RSAFromPkcs8.RSASignKeyFromFile(prestr, _private_key_file, _input_charset);
                        break;
                    }
                    break;
                default:
        ...
        {
            string url = GATEWAY_NEW + "service=query_timestamp&partner=" + AlipayConfig.Partner + "&_input_charset=" + AlipayConfig.InputCharset;
            string encrypt_key = "";
